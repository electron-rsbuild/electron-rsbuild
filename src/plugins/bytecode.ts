...
import { type Plugin, type ResolvedConfig, createFilter } from '@rsbuild/core'
import * as babel from '@babel/core'
import MagicString from 'magic-string'

import { getElectronPath } from '../electron'
import { normalizePath, toRelativePath } from '../utils'
...
function getBytecodeCompilerPath(): string {
  return path.join(path.dirname(_require.resolve('electron-rsbuild/package.json')), 'bin', 'electron-bytecode.cjs')
}
...
export function bytecodePlugin(options: BytecodeOptions = {}): Plugin | null {
  ...
  return {
    name: 'rsbuild:bytecode',
    apply: 'build',
    enforce: 'post',
    configResolved(resolvedConfig): void {
      config = resolvedConfig
      useInRenderer = config.plugins.some(p => p.name === 'rsbuild:electron-renderer-preset-config')
      if (useInRenderer) {
